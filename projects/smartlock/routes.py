...
smartlock_bp = Blueprint("smartlock", __name__)
...
@smartlock_bp.teardown_app_request
def close_db(e=None):
    db = g.pop("db", None)
    if db: db.close()
...
@smartlock_bp.context_processor
def inject_csrf():
    return dict(csrf_token=generate_csrf)

@smartlock_bp.before_request
def check_session():
    if session.get("role") == "admin":
        ...
        if token:
            ...
            if not row:
                session.clear()
                return redirect(url_for("smartlock.smartlock_login"))
...
def send_admin_magic_link(email, match_number):
    ...
    link = url_for("smartlock.smartlock_verify", token=token, _external=True)
...
def send_verification_link(new_email, match_number):
    ...
    link = url_for("smartlock.smartlock_verify_email_change", token=token, _external=True)
    token_hash = hashlib.sha256(token.encode()).hexdigest()
    db = sqlite3.connect(DB_PATH)
    db.execute("INSERT OR REPLACE INTO match_numbers (token_hash, number) VALUES (?, ?)", (token_hash, match_number))
    db.commit()
...
@smartlock_bp.route("/deploy", methods=["POST"])
@csrf.exempt
def deploy():
...
@smartlock_bp.route("/smartlock/")
def smartlock_index():
    if is_admin(): return redirect(url_for("smartlock.smartlock_admin"))
    return redirect(url_for("smartlock.smartlock_login"))

@smartlock_bp.route("/smartlock/login", methods=["GET", "POST"])
def smartlock_login():
    ...
    if remaining == 0 and match_number:
        ...
        match_number = None
    if is_admin():
        return redirect(url_for("smartlock.smartlock_admin"))
    return render_template_string(ADMIN_LOGIN_PAGE, admin_sent=bool(match_number),
                                  link_cooldown=remaining, match_number=match_number)

@smartlock_bp.route("/smartlock/poll-status")
def smartlock_poll_status():
    if is_admin(): return jsonify({"status": "logged_in"})
    return jsonify({"status": "waiting"})

@smartlock_bp.route("/smartlock/verify")
...
@smartlock_bp.route('/smartlock/verify-number', methods=['GET', 'POST'])
...
@smartlock_bp.route("/smartlock/add-session")
def smartlock_add_session():
    if not is_admin(): return redirect(url_for("smartlock.smartlock_login"))
    ...
    db.commit()
    join_url = url_for("smartlock.smartlock_join", token=token, _external=True)
    return render_template_string(ADD_SESSION_PAGE, number=number, token=token, join_url=join_url)

@smartlock_bp.route("/smartlock/join/<token>")
...
@smartlock_bp.route("/smartlock/join-verify", methods=["POST"])
...
@smartlock_bp.route("/smartlock/session/logout/<session_token>")
...
@smartlock_bp.route("/smartlock/session/logout-all")
...
@smartlock_bp.route("/smartlock/change-email", methods=["POST"])
def smartlock_change_email():
    ...
    if not new_email: return redirect(url_for("smartlock.smartlock_admin"))
    now = datetime.datetime.utcnow().isoformat()
    match_number = str(random.randint(10, 99))
    ...
    return redirect(url_for("smartlock.smartlock_email_pending"))

@smartlock_bp.route("/smartlock/change-email/resend")
def smartlock_resend_verification():
    if not is_admin(): return redirect(url_for("smartlock.smartlock_login"))
    pending = get_pending_email()
    if not pending: return redirect(url_for("smartlock.smartlock_admin"))
    now = datetime.datetime.utcnow().isoformat()
    match_number = str(random.randint(10, 99))
    session["email_change_match_number"] = match_number
...
@smartlock_bp.route("/smartlock/change-email/cancel")
...
@smartlock_bp.route("/smartlock/change-email/pending")
...
@smartlock_bp.route("/smartlock/verify-email-change")
def smartlock_verify_email_change():
    ...
    pending = get_pending_email()
    if not pending or pending != new_email: return redirect(url_for("smartlock.smartlock_admin"))
    token_hash = hashlib.sha256(token.encode()).hexdigest()
    db2 = sqlite3.connect(DB_PATH)
    db2.row_factory = sqlite3.Row
    row = db2.execute("SELECT number FROM match_numbers WHERE token_hash = ?", (token_hash,)).fetchone()
...
@smartlock_bp.route("/smartlock/verify-email-number", methods=["POST"])
...
@smartlock_bp.route("/smartlock/admin")
def smartlock_admin():
    if not is_admin(): return redirect(url_for("smartlock.smartlock_login"))
...
@smartlock_bp.route("/smartlock/users/add", methods=["POST"])
...
@smartlock_bp.route("/smartlock/users/delete/<int:user_id>")
...
@smartlock_bp.route("/smartlock/user/<int:user_id>")
def smartlock_user_detail(user_id):
    if not is_admin(): return redirect(url_for("smartlock.smartlock_login"))
    user = get_db().execute("SELECT * FROM users WHERE id = ?", (user_id,)).fetchone()
    return render_template_string(ADMIN_USER_DETAIL, user=user)

@smartlock_bp.route("/smartlock/user/<int:user_id>/toggle/<method>")
def smartlock_toggle_method(user_id, method):
    if not is_admin(): return redirect(url_for("smartlock.smartlock_login"))
    if method not in ("rfid", "fingerprint"): return redirect(url_for("smartlock.smartlock_user_detail", user_id=user_id))
    col = f"{method}_enabled"
    db = get_db()
    user = db.execute("SELECT * FROM users WHERE id = ?", (user_id,)).fetchone()
    db.execute(f"UPDATE users SET {col} = ? WHERE id = ?", (0 if user[col] else 1, user_id))
    db.commit()
    return redirect(url_for("smartlock.smartlock_user_detail", user_id=user_id))

@smartlock_bp.route("/smartlock/user/<int:user_id>/set/<method>", methods=["POST"])
...
@smartlock_bp.route("/smartlock/logout")
