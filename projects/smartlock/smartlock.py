...
def init_smartlock(app):
    ...
    def smartlock_join(token):
        if is_admin():
            return render_page("smartlock/verification_complete.html", page_name="Smart Lock — Session Active",
                                          redirect_url=url_for("smartlock_admin"),
    ...
    def smartlock_change_email():
        ...
        if error:
            ...
            return render_page("smartlock/admin_panel.html", page_name="Smart Lock", users=get_db().execute("SELECT * FROM users ORDER BY created_at DESC").fetchall(),
